augroup vim_lsp_settings_julia_language_server
  ...
  LspRegisterServer {
      ...
      \ 'cmd': {server_info->lsp_settings#get('julia-language-server', 'cmd', [lsp_settings#exec_path('julia-language-server'), matchstr(system('julia -e "using Pkg; Pkg.activate()"'),'\%(`\)\@<=.*\%(`\)\@=')])
  )},
